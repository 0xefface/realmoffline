...
namespace ROLLauncher
{
    public static class Globals
    {
        ...
        public static string ApplicationExe = Application.ExecutablePath;
        ...
        public static void RestartAsAdmin(string ip, string port, string realmDir)
        {
            // Restart program and run as admin
            ProcessStartInfo startInfo = new ProcessStartInfo(Globals.ApplicationExe);
            startInfo.Verb = "runas";
            startInfo.Arguments = string.Format("{0} {1} {2}", ip, port, realmDir);
            System.Diagnostics.Process.Start(startInfo);
        }
        public static bool WriteTestFile(string location)
        {
            bool created = false;
            try
            {
                using (File.Create(Path.Combine(location, "Text.txt")))
                {
                    created = true;
                }
                File.Delete(Path.Combine(location, "Text.txt"));
            }
            catch { created = false; }
            return created;
        }
