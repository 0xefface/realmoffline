...
namespace RealmOfflineWorldServer
{
    ...
    public class ConsoleCommands
    {
        ...
        //  public byte[] ByteStringToArray(string filename)
        //{
        ...
        public string ArrayTo0xByteString(byte[] value)
        {
            StringBuilder result = new StringBuilder();

            for(int i =0;i<value.Length;i++)
            {
                if (i == value.Length) result.Append(string.Format("0x{0}", value[i]));
                else result.Append(string.Format("0x{0}, ", value[i]));
            }
