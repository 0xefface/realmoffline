...
0x00};
    ...
    */
        ...
        public static byte[] FullMobileWrap(GameMobile mobile, Account sendTo)
        {
            PacketWriter _w = new PacketWriter(0x2A);
            _w.WriteUInt32(sendTo.AccountId);
            _w.WriteUInt32(0);
            _w.WriteInt32(-1);
            _w.WriteUInt32(sendTo.CurrentCharacter.Location.RoomNumber);

            _w.WriteByte(0x40);

            byte[] data = mobile.ToByteArray();
            _w.WriteShort((short)(data.Length + 1));
            _w.WriteUInt32(mobile.GameID);
            _w.WriteBytes(data);
            _w.WriteByte(0xFF);
            return _w.ToArray();
        }
    ...
    public static byte[] WrappedNPC(NPC n, RoomLocation l)
        ...
        public static byte[] WrappedItem(Account a, byte[] item)
        {
            uint itemID = 0;
            PacketWriter e = new PacketWriter();
            e.WriteBytes(item);
        ...
        public static byte[] WriteToPacket(Entity entity)
        {
            MemoryStream _stream = new MemoryStream();
            BinaryWriter _write = new BinaryWriter(_stream);

            _write.Write(0x01);
            _write.Write(entity.GraphicID);
            _write.Write(entity.GameID);

            if (entity is Item)
            {
                _write.Write(entity.Color);
            }

            if (!string.IsNullOrEmpty(entity.Name))
            {
                byte[] name = Encoding.ASCII.GetBytes(entity.Name);
                _write.Write((short)name.Length);
                _write.Write(name);
            }
            else _write.Write((byte)0x00);

            // buffs

            //foreach (byte b in affectedState) _write.Write(b);

            _write.Write((byte)0xFF); // end buffs

            // room location

            if (entity.Location == null) _write.Write((byte)0x00);
            else
            {
                if(entity.Location.RoomNumber < 0xFFFF)
                {
                    _write.Write((byte)0x01);
                    _write.Write((ushort)entity.Location.RoomNumber);
                }
                else
                {
                    _write.Write((byte)0x02);
                    _write.Write(entity.Location.RoomNumber);
                }
            }

            if(entity is Character)
            {
                Character o = entity as Character;

                // supposed to be 2 bytes xSpeed, ySpeed
                _write.Write((ushort)2060);
                _write.Write(o.Girth);
                _write.Write(o.Height);
                _write.Write(o.Class);
                _write.Write(o.Race);
                _write.Write(o.Gender);
                byte[] name = Encoding.ASCII.GetBytes(o.Name);
                _write.Write((short)name.Length);
                _write.Write(name);
                _write.Write(o.PvpSwitch);
                _write.Write(o.CurrentHP);
                _write.Write(o.TotalHP);
            }
            if (entity is NPC)
            {
                NPC o = entity as NPC;
