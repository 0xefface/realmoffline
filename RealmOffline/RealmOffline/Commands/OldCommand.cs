                ...
                case "/nitem":
                    {
                        ...
                        c.GraphicID = 1470;
                        c.Color = 0x44;
                        c.GameID = ServerGlobals.GetNextAvailableID();


                        GameContainer c1 = new GameContainer();
                        c1.GraphicID = 1407;
                        c1.Color = 0x41;
                        c1.GameID = ServerGlobals.GetNextAvailableID();

                        c.Items.Add(c1);

                        byte[] p = Packet.WrappedItem(Player, c.ToByteArray());
                        ...
                        byte[] pa = w42A.ToArray();
                        Player.Socket.Send(ref pa);
                    }
                    break;
                case "/nmob":
                    {
                        // New way for items work, lets try mob next
                        ...
                        m.YSpeed = 0x08;
                        m.Profession = 0x01;
                        m.Location = new GameLocation(Player.CurrentCharacter.Location.CurrentRoom.RoomID,

                            Player.CurrentCharacter.Location.X, Player.CurrentCharacter.Location.Y,
                            Player.CurrentCharacter.Location.Facing);


                        byte[] packet = Packet.FullMobileWrap(m, Player);
                        Player.Socket.Send(ref packet);

                        string fileLocation1 = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "nMob.txt");
                ...
                case "/nnpc":
                    {
                        ...
                        m.Head = new GameMobileHead();
                        m.Head.Race = 0x01;
                        m.Head.Gender = 0x01;
                        m.Head.Head = 0x01;
                        m.Head.Hair = 0x00;

                        m.Inventory = new List<GameItem>();
                ...
                case "/additem":
                    {
                        ...
                        if (CommandArgs.Length >= 2)
                        {
                            ...
                            if (ushort.TryParse(CommandArgs[1], out id))
                            {
                                ...
                                i.IsContainer = false;
                                i.IsEquipable = false; // open and close for a door
                            ...
                            if (uint.TryParse(CommandArgs[1], out roomNum))
                            {
                              ...
                              string.Format("Room {0} is in use.", roomNum), "CreateRoom");
                                ...
                                else
                                {
                                    ...
                                    prop.Background = Room.RoomBackground.PlainDirt;
                        ...
                        else
                        {
                            ...
                            if(short.TryParse(CommandArgs[1], out mobid))
                            {
                              ...
                              // add mob to room
                                ...
                                string fileLocation1 = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "cmdMobWrap.txt");
                                File.WriteAllText(fileLocation1, BitConverter.ToString(try1));
                                Console.WriteLine("Sent Data");
                            }
                            else
                            {
